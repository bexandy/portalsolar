...
function aitGetTrialPackageUserDaysLeft($userId, $packageExpirationDays){
	// check how many days till user account expires
	// userId must be a user with "cityguide_" user role
	// return days | 0 -> expired
	$result = 0;
	$user_meta = get_user_meta( $userId, 'package_trial_activation_time', false );
	$user_meta = reset($user_meta);
...
function aitSetPackageUserActivationTime($userId, $role){
	if(isThemeUserRole($role)){
		...
		if ($trialStatus['status'] == 'activated') {
			...
			delete_user_meta( $userId, 'package_trial_activation_time' );
		}
...
function showUserAccountInformation($user){
	...
	if(isThemeUserRole($role)){
		...
		<table class="form-table">
			...
			<tr>
				<th><label for="account_trial_expiration"><?php _e('Trial Expiration in days', 'ait-admin') ?></label></th>
				<td><input type="text" name="account_trial_expiration" value="<?php echo $trialDaysLeft ?>" class="regular-text" disabled="disabled" /></td>
			</tr>
